...
class RevenueCatService {
  ...
  async getOfferings(): Promise<SubscriptionPlan[]> {
    try {
      ...
      if (!currentOffering) {
        throw new Error('No current offering configured in RevenueCat. Please configure offerings in your RevenueCat dashboard.')
      }

      if (!currentOffering.availablePackages || currentOffering.availablePackages.length === 0) {
        throw new Error('No subscription packages available. Please configure products in your RevenueCat dashboard.')
      }
