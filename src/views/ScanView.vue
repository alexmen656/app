<template>
    <div class="scan-view" :class="{ 'barcode-mode': scanMode === 'barcode' }">
        ...
        <div class="camera-container" v-if="!scanResults && !error">
            ...
            <div class="bottom-controls">
                ...
                <div class="scan-mode-toggle">
                    <button class="mode-btn" :class="{ active: scanMode === 'barcode' }"
                        @click="setScanMode('barcode')">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                            <path
                                d="M3 5v4h2V7h2V5H3zm2 10H3v4h4v-2H5v-2zm14 4v-2h-2v4h4v-4h-2zM19 5h-2v2h2v2h2V5h-4z" />
                        </svg>
                    </button>
                    <span class="mode-label">{{ modeLabel }}</span>
                    <button class="mode-btn" :class="{ active: scanMode === 'food' }" @click="setScanMode('food')">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                            <path
                                d="M9 2l-1 1H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2h-4L15 2H9zm3 15c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5z" />
                        </svg>
                    </button>
                </div>
                ...
                <div class="capture-controls">
                    <div class="capture-actions">
                        ...
                        <!-- Main Capture Button -->
                        <button class="capture-btn" :disabled="isScanning || isAnalyzing"
        ...
        <div class="results" v-if="scanResults">
                ...
                <div class="main-card">
                    ...
                    <div class="progress-ring">
                        <svg class="progress-svg" width="80" height="80" viewBox="0 0 80 80">
                            ...
                            <circle cx="40" cy="40" r="30" stroke="white" stroke-width="4" fill="none"
                                stroke-dasharray="188.4" stroke-dashoffset="0" stroke-linecap="round"
                ...
                <div class="macros-grid">
                    <div class="macro-card protein">
                        ...
                        <div class="macro-progress">
                            <svg width="50" height="50" viewBox="0 0 50 50">
                                ...
                                <circle cx="25" cy="25" r="20" stroke="#ff6b6b" stroke-width="3" fill="none"
                    ...
                    <div class="macro-card carbs">
                        ...
                        <div class="macro-progress">
                            <svg width="50" height="50" viewBox="0 0 50 50">
                                <circle cx="25" cy="25" r="20" stroke="#2a2d37" stroke-width="3" fill="none" />
                                <circle cx="25" cy="25" r="20" stroke="#ffa726" stroke-width="3" fill="none"
                                    stroke-dasharray="125.6" stroke-dashoffset="0" stroke-linecap="round"
                                    transform="rotate(-90 25 25)" />
                            </svg>
                            <div class="macro-icon">
                    ...
                    <div class="macro-card fats">
                        ...
                        <div class="macro-progress">
                            <svg width="50" height="50" viewBox="0 0 50 50">
                                <circle cx="25" cy="25" r="20" stroke="#2a2d37" stroke-width="3" fill="none" />
                                <circle cx="25" cy="25" r="20" stroke="#42a5f5" stroke-width="3" fill="none"
                                    stroke-dasharray="125.6" stroke-dashoffset="0" stroke-linecap="round"
                                    transform="rotate(-90 25 25)" />
                            </svg>
                            <div class="macro-icon">
                                <svg width="12" height="12" viewBox="0 0 24 24" fill="#42a5f5">
                                    <path
                                        d="M12 2c5.514 0 10 4.486 10 10s-4.486 10-10 10-10-4.486-10-10 4.486-10 10-10zm0 2c-4.411 0-8 3.589-8 8s3.589 8 8 8 8-3.589 8-8-3.589-8-8-8z" />
                                </svg>
                            </div>
                        </div>
                    </div>
            ...
            <div v-if="scanResults.type === 'barcode'" class="barcode-results">
                <div class="main-card">
                    <div class="calories-section">
                        <h2 class="calories-number">{{ Math.round(scanResults.data.nutriments?.energy_kcal_100g || 0) }}
                        </h2>
                ...
                <div class="macros-grid" v-if="scanResults.data.nutriments">
                    <div class="macro-card protein">
                        <div>
                            <div class="macro-amount">{{ (scanResults.data.nutriments.proteins_100g || 0).toFixed(1) }}g
                            </div>
                    ...
                    <div class="macro-card carbs">
                        <div>
                            <div class="macro-amount">{{ (scanResults.data.nutriments.carbohydrates_100g ||
                                0).toFixed(1) }}g</div>
                    ...
                    <div class="macro-card fats">
                        <div>
                            <div class="macro-amount">{{ (scanResults.data.nutriments.fat_100g || 0).toFixed(1) }}g
                            </div>
...
const isBarcodeListenerActive = ref(false)

// Video element reference (for food scanning only)
...
function setScanMode(mode: 'food' | 'barcode') {
    // If switching from barcode mode, stop the barcode scanner
    if (scanMode.value === 'barcode' && mode !== 'barcode') {
        stopBarcodeScanning()
    }
...
async function toggleFlash() {
    try {
        if (scanMode.value === 'barcode' && Capacitor.isNativePlatform()) {
            // Use Torch plugin for barcode scanning mode
            ...
            // Use camera flash for food scanning mode
...
// Barcode scanning functions using MLKit live scanning
async function startBarcodeScanning() {
    if (!Capacitor.isNativePlatform()) {
        error.value = 'Barcode-Scanning ist nur auf mobilen Geräten verfügbar'
        return
    }
    ...
    try {
        ...
        if (!supported) {
            throw new Error('Barcode-Scanner wird auf diesem Gerät nicht unterstützt')
        }

        // Check permissions
        const { camera: cameraPermission } = await BarcodeScanner.checkPermissions()
        if (cameraPermission !== 'granted') {
            const { camera: requestedPermission } = await BarcodeScanner.requestPermissions()
            if (requestedPermission !== 'granted') {
                throw new Error('Kamera-Berechtigung ist erforderlich für Barcode-Scanning')
            }
        }

        // Add the scanner active class for styling
        document.querySelector('body')?.classList.add('barcode-scanner-active')

        // Add the barcode scanned listener
        barcodeListener = await BarcodeScanner.addListener(
            ...
            async (result: any) => {
                ...
                if (result.barcodes && result.barcodes.length > 0) {
                    ...
                    await handleBarcodeDetected(firstBarcode.rawValue)
                }
            }
        )
        ...
        isBarcodeListenerActive.value = true
        console.log('Barcode scanner started successfully')

    } catch (err: any) {
        console.error('Error starting barcode scanner:', err)
        error.value = err.message || 'Fehler beim Starten des Barcode-Scanners'
...
async function stopBarcodeScanning() {
    try {
        // Remove the scanner active class
        ...
        if (isBarcodeListenerActive.value) {
            await BarcodeScanner.stopScan()
            isBarcodeListenerActive.value = false
        }
...
async function handleBarcodeDetected(barcodeValue: string) {
    if (!barcodeValue || isAnalyzing.value) {
        return // Ignore if already processing or no value
    }
    ...
    try {
        ...
        const productResponse = await fetch(`https://world.openfoodfacts.org/api/v0/product/${barcodeValue}.json`)
        ...
        error.value = err.message || 'Fehler beim Verarbeiten des Barcodes'
    } finally {
...
// New simplified barcode scan trigger
...
function resetScan() {
    ...
    // If in barcode mode, restart the scanner
    if (scanMode.value === 'barcode') {
        setTimeout(() => {
            startBarcodeScanning()
        }, 100)
...
:global(body.barcode-scanner-active .scan-view) {
    visibility: visible;
    background: transparent !important;
}

:global(body.barcode-scanner-active .scan-view .camera-container) {
    background: transparent !important;
}

:global(body.barcode-scanner-active .scan-view .camera-view-full) {
    background: transparent !important;
}

:global(body.barcode-scanner-active .scan-view .camera-preview) {
    background: transparent !important;
}

:global(body.barcode-scanner-active .scan-view .header-bar) {
    visibility: visible;
    background: linear-gradient(180deg, rgba(0, 0, 0, 0.8) 0%, rgba(0, 0, 0, 0.4) 50%, transparent 100%);
}

:global(body.barcode-scanner-active .scan-view .bottom-controls) {
    visibility: visible;
}

:global(body.barcode-scanner-active .scan-view .viewfinder-overlay) {
    visibility: visible;
}
